...
export default function MovieSearch({
	...
	const [searchResults, setSearchResults] =
		useState<SearchMoviesReturnType | null>(null);

	useEffect(() => {
		if (debouncedSearchTerm) {
			startTransition(async () => {
				const results = await searchMovies({ query: debouncedSearchTerm });
	...
	return (
		<div className="max-w-[400px] w-full">
			...
			<div className="bg-surface-50 mt-4 py-3 border border-dashed shadow-wg-xs overflow-y-auto max-h-[160px] flex flex-col justify-between items-center w-full min-h-[160px] rounded-lg">
				...
				{searchResults && !isPending && searchResults.results.length > 0 ? (
					<TooltipProvider>
