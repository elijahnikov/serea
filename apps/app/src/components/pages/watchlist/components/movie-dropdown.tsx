...
import { toggleWatchedAction } from "~/actions/watched/toggle-watched";
import { toggleAllWatchedAction } from "~/actions/watched/toggle-all-watched";
...
export default function MovieDropdown({
	...
	entry: NonNullable<GetWatchlistEntriesReturnType>[number];
	onDeleteEntry: (entryId: string) => void;
	onOpenChange: (isOpen: boolean) => void;
	role: "owner" | "editor" | "viewer" | "non-member";
	isOpen: boolean;
}) {
	const toggleWatch = useAction(toggleWatchedAction);
	const toggleAllWatched = useAction(toggleAllWatchedAction);
		...
		() => [
			...
			},
			{
				...
				onSelect: (watchlistId: string, entryId: string) =>
					toggleWatch.execute({ watchlistId, entryId }),
			},
			{
				label: "Mark as watched for all",
				icon: <UserCheckIcon size={16} />,
				onSelect: (watchlistId: string, entryId: string) =>
					toggleAllWatched.execute({ watchlistId, entryId }),
			},
			{
