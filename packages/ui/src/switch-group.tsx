...
import type { SwitchElement, SwitchProps } from "./switch";
...
/* --------------------------------- Context -------------------------------- */
const SwitchGroupContext = React.createContext<SwitchGroupContextProps | null>(
	null,
);
...
export function useSwitchGroupContext(skipCheck = false) {
	...
	if (!context && !skipCheck) {
		throw new Error(
			"SwitchGroup.Item must be used within a SwitchGroup or SwitchGroup.Root",
		);
	}
...
/* ------------------------------- Components ------------------------------- */
const SwitchGroupWedges = React.forwardRef<
	SwitchGroupElement,
	SwitchGroupProps
>(
	(
...
>(({ label, className, disabled, ...otherProps }, ref) => {
	...
	return (
		<Switch
			...
			className={cn(
				"wg-switch-group__item",
				alignLabels === "start" && "flex w-full justify-between",
				className,
			)}
			disabled={isDisabled}
			label={label}
