		...
		<DayPicker
			...
			classNames={{
				...
				button_previous: cn(
					...
					"h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100 absolute left-5 top-6",
					disableLeftNavigation() && "pointer-events-none",
				),
				button_next: cn(
					buttonVariants({ variant: "outline" }),
					"h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100 absolute right-5 top-6",
				...
				MonthCaption: ({ calendarMonth }) => {
					return (
						<div className="inline-flex gap-2">
							...
							>
								<SelectTrigger className="gap-1 focus:bg-accent focus:text-accent-foreground">
									<SelectValue />
								</SelectTrigger>
								<SelectContent>
									{MONTHS.map((month) => (
...
>(
	(
		...
		return (
			<div className="dark:bg-carbon-dark-200 border shadow-overlay dark:shadow-overlay-dark rounded-lg">
				<Calendar
					mode="single"
