...
import { cn } from ".";
import Label, { type LabelHelperProps, type LabelProps } from "./label";
	...
	return (
		<SelectPrimitive.Trigger
			...
			className={cn(
				...
				// open state
				"[&[data-state=open]_.wg-select-icon]:rotate-180 [&_.wg-select-icon]:transition-transform [&_.wg-select-icon]:duration-200",
...
>(({ className, ...props }, ref) => (
	<SelectPrimitive.Label
		ref={ref}
		className={cn(
			"w-full px-4 py-1 text-start text-sm leading-6 text-surface-500",
			className,
		)}
		{...props}
	/>
...
const Select = React.forwardRef<SelectElement, SelectProps>((props, ref) => {
	...
	return (
		<div
			ref={ref}
			className={cn("flex flex-col gap-2 wg-antialiased", className)}
		>
			<Label
